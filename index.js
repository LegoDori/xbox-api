...
exports.getprofilerecentgames = async function(xuid, authorization) {
    try {
    var fetched = await fetch('https://titlehub.xboxlive.com/users/xuid(' + xuid + ')/titles/titlehistory/decoration/', { method: 'GET', headers: {'x-xbl-contract-version': '2', 'Authorization': 'XBL3.0 x=' + authorization.userHash + ';' + authorization.XSTSToken, "Accept-Language": "en-GB" }}).then(response => response.json())
    return fetched
    } catch {
        return 'null'
    }
...
exports.getownrecentgames = async function(authorization) {
    try {
    var fetched = await fetch('https://titlehub.xboxlive.com/users/me/titles/titlehistory/decoration/', { method: 'GET', headers: {'x-xbl-contract-version': '2', 'Authorization': 'XBL3.0 x=' + authorization.userHash + ';' + authorization.XSTSToken, "Accept-Language": "en-GB" }}).then(response => response.json())
    return fetched
    } catch {
        return 'null'
    }
